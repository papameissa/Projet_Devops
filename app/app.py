...
# ── Healthcheck ──────────────────────────────────────────────
# /health = liveness : le process Flask répond, SANS dépendance externe.
# Une coupure DB ne doit pas faire redémarrer ce conteneur par kubelet.
@app.route("/health")
def health():
    return jsonify({"status": "ok"}), 200


# /ready = readiness : vérifie la DB. Si elle échoue, le pod est retiré
# du Service (plus de trafic routé) mais n'est PAS redémarré.
@app.route("/ready")
def ready():
    try:
